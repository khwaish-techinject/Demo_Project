...
const server = new McpServer(
  {
    name: "datapilot-erp-mcp-server",
    version: "1.0.0",
  },
  {
    instructions:
      "Use the provided resources and tools to inspect the ERP database schema and answer business questions. Prefer schema-aware ERP tools first, and use run_sql_query for read-only SQL when necessary.",
  }
);
...
server.registerTool(
  ...
  {
    description:
      "Execute a read-only SQL query on PostgreSQL for ERP reporting and analysis. Only SELECT and WITH queries are allowed.",
    inputSchema: {
...
server.registerTool(
  "list_customers",
  {
    description: "List customers from the ERP database for AI lookups and reporting.",
    inputSchema: {
...
server.registerTool(
  "list_products",
  {
    description: "List products from the ERP database for AI lookups and pricing questions.",
    inputSchema: {
...
server.registerTool(
  "list_sales",
  {
    description: "List sales rows from the ERP database for analysis and summaries.",
    inputSchema: {
...
async function main() {
  ...
  console.error("DataPilot ERP MCP server is running on stdio.");
}
